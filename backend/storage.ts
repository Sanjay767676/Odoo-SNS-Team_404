...
export interface IStorage {
  ...
  setUserOTP(id: string, otp: string, expiry: Date): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getInternalUsers(_companyId: string | null): Promise<User[]> {
    // TEMPORARY: Return all internals for debugging
  ...
  async setUserOTP(id: string, otp: string, expiry: Date): Promise<void> {
    await db.update(users).set({ otp, otpExpiry: expiry }).where(eq(users.id, id));
  }
