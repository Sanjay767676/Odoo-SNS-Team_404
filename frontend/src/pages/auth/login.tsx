...
const signupSchema = z.object({
  ...
  role: z.enum(["admin", "user", "internal"]),
});
...
export default function LoginPage() {
  ...
  const onLogin = async (data: LoginForm) => {
    ...
    try {
      ...
      // No need to toast here if we want to be clean, but user asked to check "welcome back" logic.
      // If the existing user returns display welcome back.
  ...
  const onGoogleSuccess = async (credentialResponse: any) => {
    try {
      ...
      // isNew is returned from backend to distinguish first-time Google signups
      const title = res?.isNew ? "Welcome!" : "Welcome back!";
      toast({ title, description: "Logged in with Google successfully." });
    } catch (err: any) {
      // Handled by global toast
    }
  };
            ...
            <CardContent>
              {!isSignup ? (
                <form onSubmit={loginForm.handleSubmit(onLogin)} className="space-y-4">
                  ...
                  <div className="relative my-4 text-center text-xs after:absolute after:inset-0 after:top-1/2 after:z-0 after:flex after:items-center after:border-t after:border-border">
                    <span className="relative z-10 bg-background px-2 text-muted-foreground uppercase">Or continue with</span>
                  </div>
